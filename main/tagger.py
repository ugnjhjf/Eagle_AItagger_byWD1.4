...
class TaggerService:
    ...
    def process_tags(self, raw_tags: Dict[str, float]) -> Dict[str, float]:
        ...
        if self.config.tag.additional_tags:
            tags.update({tag: 1.0 for tag in self.config.tag.additional_tags if tag not in tags})

        filtered = {tag: conf for tag, conf in tags.items() if conf >= self.config.tag.threshold}

        if self.config.tag.exclude_tags:
            filtered = {tag: conf for tag, conf in filtered.items() if tag not in self.config.tag.exclude_tags}
        
        # 过滤非法字符
        illegal_chars = {'[', ']', ',', '(', ')', '\\'}
        valid_tags = {tag: conf for tag, conf in filtered.items() if not (len(tag) == 1 and tag in illegal_chars)}

        # 排序
        sorted_tags = sorted(valid_tags.items(), 
                             key=lambda x: (-x[1], x[0]) if not self.config.tag.sort_alphabetically else x[0])
