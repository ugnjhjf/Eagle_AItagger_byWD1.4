...
class ResultCollector:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        avg_tags_per_image = self.total_tags / self.success_count if self.success_count > 0 else 0
        success_rate = (self.success_count / total_images * 100) if total_images > 0 else 0
        return {
    ...
    def get_failed_images(self) -> List[Dict]:
        return [
            ...
            for r in self.results if not r.get('success', False)
        ]
    ...
    def generate_report(self):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            if failed:
                f.write("\n失败图像列表:\n")
                for f in failed:
                    f.write(f"- {f['image_path']}: {f['error']}\n")
